#!/bin/bash
# no-chain ablation: heads receive h only (E2E_CHAIN=none). seeds 42 -> 44 -> 45, sequential.
cd ~/TS_code
...
run_seed () {
  local SEED=$1
  local DIR=result_e2e3_bb1_nochain_s${SEED}
  local OUT=~/TS_code/${DIR}/e2e_${M}_${NF}f_test_finetuning_bb1_eval.json
  if [ -e "$OUT" ]; then echo "⏭️   [nochain s$SEED] 완료됨 → 건너뜀"; return 0; fi
  echo ""
  echo "════ 🎲 [nochain $M ${NF}f seed $SEED] 학습  $(date '+%m-%d %H:%M') ════"
  ...
  E2E_RESULT_DIR=$DIR python -m torch.distributed.run --nproc_per_node=4 e2e_train3_ms.py \
      ...
      2>&1 | tail -8
  if [ -e "$OUT" ]; then echo "✅ [nochain s$SEED] 완료 $(date '+%m-%d %H:%M')"; else echo "❌ [nochain s$SEED] 결과 JSON 없음 — 실패, 다음 시드 진행" | tee -a nochain_failures_ms.log; fi
}
