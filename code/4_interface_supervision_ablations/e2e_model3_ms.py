...
class TripleHeadFaultModel(nn.Module):
    ...
    def __init__(self, backbone: VLMBackbone):
        ...
        dev = backbone.device
        import os as _os
        self.nochain = _os.environ.get('E2E_CHAIN', 'soft') == 'none'   # no-chain ablation: heads see h only
        cmp_extra = 0 if self.nochain else N_SIT
        reg_extra = 0 if self.nochain else N_SIT + 3
        ...
        self.cmp_head = nn.Sequential(
            nn.Linear(hs + cmp_extra, hs // 2), nn.GELU(), nn.Dropout(0.3),
            nn.Linear(hs // 2, 3)).to(dev).to(torch.bfloat16)
        # 비율헤드: hidden + 상황확률(13) + 비교확률(3) → 11개 (분류)
        self.reg_head = nn.Sequential(
            nn.Linear(hs + reg_extra, hs // 2), nn.GELU(), nn.Dropout(0.3),
    ...
    def forward(self, frames, prompt):
        ...
        # 2) 비교 (hidden + 상황확률)
        cmp_in = h if self.nochain else torch.cat([h, sit_soft], dim=-1)         # (1, hs+13) / nochain: (1, hs)
        ...
        reg_in = h if self.nochain else torch.cat([h, sit_soft, cmp_soft], dim=-1)  # (1, hs+13+3) / nochain: (1, hs)
