...
def main():
    ...
    import e2e_model3 as _m3
    _m3.set_ratio_classes(args.ratio_classes)
    global RATIO_VALUES, N_RATIO
    from e2e_model3 import RATIO_VALUES, N_RATIO
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, i in enumerate(pbar):
            ...
            loss = (args.lambda_sit * loss_sit + loss_cmp + args.lambda_reg * loss_reg) / args.grad_accum
            loss.backward()
        ...
        if is_main and v:
            score = v["cmp_macro_f1"] + v["ratio_within10"]   # 균형: 비교F1 + 비율within10
